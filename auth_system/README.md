...
- 当前 C++ 客户端已经改为只接收服务端下发的完整 `hfy` 文件。
- 客户端不再读取本地 `template.hfy`，也不再在本地生成授权文件。
- 服务端必须配置 `HFY_TEMPLATE_PATH`，否则激活接口会直接返回失败。
...
- `FIXED_TEXT` 兼容：
  - 服务端按严格格式生成
  - 客户端兼容你给出的 `hfy` 加解密逻辑
  - 如果服务端配置了 `HFY_TEMPLATE_PATH`，服务端可直接返回完整加密 blob
  - 如果服务端未配置模板，客户端会读取本地 `template.hfy` 生成 `C:\Windows\hfy`
...
```text
产品名|添加时间|到期时间|提示语|绑定IP|0|1|1|绑定IP|1|绑定QQ
```

例如：

```text
099冒险岛-自定义属性|2023/12/17 22:01:59|2135/1/1 10:46:14|本程序仅做学习交流之用，不得用于商业用途！如作他用所承受的法律责任一概与作者无关（下载使用即代表你同意上述观点）|183.141.76.29|0|1|1|183.141.76.29|1|475862105
```
...
- `template_path=template.hfy`
- `blob_key=heifengye111`

3. 如果服务端没有配置 `HFY_TEMPLATE_PATH`，请把原始模板文件放到：

```text
auth_system/client/AuthClient/template.hfy
```

或者在 `auth_client.ini` 里修改 `template_path`。
...
- `C:\Windows\hfy` 写入通常需要管理员权限
- 若要生成和原始程序完全兼容的 `hfy` 文件，需要可用的模板 `template.hfy`
